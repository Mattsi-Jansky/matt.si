...
Since I wrote ['ChatGPT Can Do Astonishing Things. But Is It a Helpful Development Tool?'](/2022-12/chatgpt/) several years ago, the world has moved on. At the time my colleagues and I were putting ChatGPT's code generation capabilities through the ringer, and I was catalogueing the results. But what was astonishing then has become commonplace now, and I'm increasingly seeing people relying on LLMs for code generation, in frequently problematic ways. But more and more people are swearing by Claude Code, saying that it achieves a very high level of competence. So I felt it was coming time to revisit this question, and just as I did Claude's creators Anthropic sent out a load of free credits with their newest model release so I figured hey, this is the time to try it out.

I'll start by delving into some light theory of how the latest models work and what has changed, and then review what I found in practice trying it across a few side projects.

## What has changed

The biggest difference between now and 2023 is that Large Language Models have been upended by Large Reasoning Models. The distinction smells like marketing spiel but it is not wholly disingenuous, while LRMs are still fundamentally LLMs they have made significant advancements over the original models, making leaps forward in performance on certain benchmarks[^1]. We may note that the bar to pass over was very low, but LRMs do appear to have improved over LLMs in leaps and bounds regarding planning and reasoning.

To understand what separate LRMs we have to understand that, similarly to how programming languages have compile-time and run-time, LLMs have training-time and inference-time. Training is when the model is trained on an enormous corpus of sources, inference is when the model (the artifact that results from training) is actually calculating how to respond to a given prompt. The key difference in the LRM approach is that you spend much more compute at inference time. This is done by calling the model continuously, with multiple steps that form a 'chain of thought', until a conclusion is reached. Each time you call the model it generates text, but much of the 'reasoning' output can be discarded because we just want the final outcome.

Going back to Professor Subbarao Kambhampati of the School of Computing & AI at Arizona State University who I referenced in ['Large Language Models Are Drunk at the Wheel'](/2024-02/llms-overpromised/), he has continued to study and write compellingly about the question of whether or not LLMs can reason. One of his key arguments rests on fundamentally how LLMs work: you query them for the next token (a 'token' is a small chunk of text, usually less than one word),  and they produce it **in constant time**. By constant time we mean that if you run the algorithm thousands of times, each run will take more-or-less the same amount of time. Kambhampati points out: if the model is really reasoning, it would take different amounts of time to solve big complex problems versus simple easy problems, but it doesn't[^2]. LRMs address this by getting the models to, instead of outputting an answer, output the low-level reasoning required to reach an answer, and then later the answer drawn from that reasoning. In essence, though each token is constant-time, this higher-level orchestration of token generation (known as an inference loop) makes the whole end-to-end process no longer constant-time, because models will spend more time discussing the reasoning for complex problems than for simple ones.

<figure src="speaking.png" alt="A scene from Star Wars: The Phantom Menace featuring Qui-Gon Jin and Jar Jar Binks walking along a forest path, with subtitle text reading 'The ability to speak does not make you intelligent.'">Wise words to keep in mind when it comes to LLMs</figure>
...
If the reasoning were genuine reasoning rather than a sort of post-hoc justification you would expect it to think more, not less about complex problems. But if it were inventing post-hoc narratives then it makes sense that simpler problems would have longer reasoning chains because the model is able to explain it, whereas it may struggle to deeply explain a complex problem and so cuts the reasoning short.

### Claude Code

On top of all these LRM changes, the other big element that has appeared is Claude Code. This is a sort of light orchestration layer that provides tools for the model to use to interact with your codebase, and prompts the model to use more structured approaches with the available tools. Models then tackle problems through several different stages: Gather information, then take action, then verify the results. For more complex problems the model will often generate a detailed plan first, verify that the user agrees with the plan, then clear it's context and start implementing the plan from a blank slate. It can also launch 'sub-agents' the solve specific tasks. All of these capabilities reflect a related simple truth: These models work better with small, compartmentalised problems and prompts that are minimal and focused, while still being explicit. A lot of what Claude Code does seems to come down to breaking problems down into smaller pieces and working through those pieces methodically, some model instances operating as orchestrators that delegate small tasks to other model instances with less context.

Another big change Claude Code gives you is an autonomous feedback loop. If some tests are broken and the model can run the tests, you can leave the model to run the test and make changes repeatedly until it can solve the problem. The ability to self-verify means that mistakes are less costly, because it can see that it made a mistake and try something else without waiting for human feedback. Of course, if it fundamentally cannot solve the problem you'll just be wasting tokens.

## Trying out Claude Code

I tried Claude Code with two main side projects: A cooking/recipe app in Typescript/React, which is a quite typical usecase that Claude Code is designed for, and some complex mesh generation and game development in Rust and Bevy, which is very much not what Claude Code is typically used for. I only initially planned to use it on the former, but after seeing how well it worked I became curious to see how it would work on a much more complex problem.

I'll start by giving you a run-down of what I was able to do with it, and then talk more about what insights I gleaned from the process.

### Cooking app

The idea behind [cooking app](https://github.com/Mattsi-Jansky/2ez-cook-client) was to take my [my recipes](https://github.com/mattsi-jansky/recipes) and put them in an interactive format that can tell you what to do when, run timers for you, and manage parallelisation too. I don't like the common format of most recipes and long had this idea to make an app that makes cooking stress-free, methodical, and fun. Most of all, I want to switch my brain off when cooking and just do what the instructions say.

<figure src="2ezcook.png">A mobile cooking app interface with an active recipe for Veggie Takoyaki, showing step 5 of 6 in the 'Dashi & Mushrooms' phase, with a circular countdown timer at 9 minutes 47 seconds, pause and skip controls, and a prompt to switch to the Fried Mushrooms task.</figure>

Above is a screenshot of what it came out with. I didn't give it any starting place and didn't specify the tech stack. I didn't give it any UI designs or give it much UX guidance other than saying that I want a calming, relaxed vibe. However I did start in Claude's web interface, conversationally discussing the idea and getting Claude itself to write a spec. Then I put that spec in an empty folder and directed Claude Code to it. After a while of playing around it generated an impressive proof-of-concept application with a cute, calm, minimalist aesthetic. I didn't really have many complaints or things to fix at all from the outset, functionality-wise. I can see from git history that my following changes were almost all about prompting Claude to improve the test coverage, refactor certain files to simplify the code, and restructuring the directory layout. From there I prompted it to add various features such as a generic recipe format so that I could throw different recipes at this thing and see how it behaves, and again its implementation was sound. I reviewed the changes carefully and prompted it to fix issues such as leaving usages of the any type around and missing typecheck errors, but overall it was a big time saver. It was nothing I couldn't have done myself but I certainly couldn't have done it so fast.

### Voxel geometry, game development

After seeing its capabilities with a simple web app, I earned a persistent desire to see what it could do with something more challenging. I have been doing amateur game development for as long as I've been programming, and in particular in the last maybe 7 years or so I've had an on-again, off-again relationship with voxel terrain systems. I've gone and written maybe 4 or 5 different voxel terrain engines in several game engines, I just find it a really interesting and fun area to work in. And I'd fantasise about making a full game using my pet projects, but mostly it was only for the joy of the hunt, the pleasure of coding something cool and solving interesting problems. So, what if I tried to get Claude to do the same? _This will be it's downfall!_ I told myself, _simple web development may be one thing, but here is something that will surely be beyond your capabilities!_. But...

<figure src="voxels.png">A voxel terrain editor with smooth slopes, foliage, and buildings. The foliage are separate procedurally generated meshes, the buildings are a part of the voxel chunk mesh just with different sizes/shapes to other terrain blocks.</figure>

It turns out, Claude is pretty great at computer graphics. At least, when you know what you're doing and use it right. I've got a good amount of experience, though not professional experience, with voxel geometry computer graphics programming so I was able to lead it quite closely - I don't mean to suggest that just anyone could write a simple prompt and get a similar result, it was still a very significant amount of personal work to get this far but even from the very first steps I was impressed.

I started similarly to the cooking app, by talking to the Claude web interface to iterate on a spec describing what I wanted. It included milestones I'd specified, and I would review and commit after each milestone. The very first one was just rendering a few blocks of different shapes in a scene, and conjuring up good abstractions for rendering those blocks. Next arrange them into a single chunk of terrain that renders as a single mesh, and finally create a whole scene by arranging chunks together. Each milestone had a separate binary that I could inspect and verify, and I ended up adding visual regression tests to some of them (more on that later).

I was surprised just how similar the code was to what I'd written myself previously. It had very similar abstractions, interfaces and naming but some neat approaches here and there that I either hadn't considered, or had disregarded as too time-consuming. Even before I spent some time going through prompting it to re-architect some parts and improve test coverage and readability, the code from the outset was impressive.
